...
class TestPddlPredicateDto(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.pddl_predicate_dto = PddlPredicateDto("robot_at", [robot_type, wp_type])

    def test_pddl_predicate_dto_str(self):
        self.assertEqual("(robot_at ?r0 - robot ?w1 - wp)", str(self.pddl_predicate_dto))

    def test_pddl_predicate_dto_str_no_types(self):
        self.pddl_predicate_dto.set_types(None)
        self.assertEqual("(robot_at)", str(self.pddl_predicate_dto))

    def test_pddl_predicate_dto_get_name(self):
        self.assertEqual("robot_at", self.pddl_predicate_dto.get_name())
