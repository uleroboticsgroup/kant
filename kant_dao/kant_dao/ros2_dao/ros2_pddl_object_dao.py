...
from kant_msgs.srv import UpdatePddlObject, GetPddlObject
...
class Ros2PddlObjectDao(PddlObjectDao):
    ...
    def __init__(self, node: Node):
        ...
        # srv clients
        self._get_client = self.node.create_client(GetPddlObject, "get_objects")

        self._update_client = self.node.create_client(UpdatePddlObject, "update_object")

        self._delete_all_client = self.node.create_client(Empty, "delete_all_objects")
    ...
    def _ros2_get(self, object_name: str = "") -> List[PddlObjectDto]:
        ...
        for pddl_object_msg in result.pddl_objects:
            pddl_object_dto = self.msg_dto_parser.object_msg_to_dto(pddl_object_msg)
            pddl_object_dto_list.append(pddl_object_dto)

        return pddl_object_dto_list

    def _ros2_update(self, pddl_object_dto: PddlObjectDto, update_type: int) -> bool:
        ...
        req.pddl_object = self.dto_msg_parser.object_dto_to_msg(pddl_object_dto)
    ...
    def _save(self, pddl_object_dto: PddlObjectDto) -> bool:
        ...
        if not self.get(pddl_object_dto.get_name()):
            succ = self._ros2_update(pddl_object_dto, UpdateKnowledge.SAVE)
            return succ
    ...
    def delete(self, pddl_object_dto: PddlObjectDto) -> bool:
        ...
        succ = self._ros2_update(pddl_object_dto, UpdateKnowledge.DELETE)
        return succ
